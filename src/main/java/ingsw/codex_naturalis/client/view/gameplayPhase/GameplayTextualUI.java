...
import ingsw.codex_naturalis.common.exceptions.EmptyGoldCardsDeckException;
import ingsw.codex_naturalis.common.exceptions.EmptyResourceCardsDeckException;
import ingsw.codex_naturalis.common.exceptions.ZeroCardsLeftException;
...
public class GameplayTextualUI extends GameplayUI {
            ...
            if (game.player().hand().size() >= option) {
                switch (option) {
                    case 1 -> {
                        ...
                        notifyFlipCard(FlipCardEvent.FLIP_CARD_1);
                    }
                    case 2 -> {
                        setState(State.WAITING_FOR_UPDATE);
                        notifyFlipCard(FlipCardEvent.FLIP_CARD_2);
                    }
                    case 3 -> {
                        setState(State.WAITING_FOR_UPDATE);
                        notifyFlipCard(FlipCardEvent.FLIP_CARD_3);
                    }
                    default -> {
                        printErrInvalidOption();
                        getFlipCardOption();
    ...
    private void sendingMessage() {
        ...
        if (playersToText.size() > 1) {
            ...
            receiver = getSendMessageOption();
            if (receiver != null && receiver.equals("/"))
                return;
        } else {
            receiver = playersToText.getFirst();
        }
